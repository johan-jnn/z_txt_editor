{
  const ICON = /*html*/ `
  <svg
    viewBox="-0.45 0 60.369 60.369"
    xmlns="http://www.w3.org/2000/svg"
  >
    <span xmlns="" id="uas-port" />
    <link
      xmlns=""
      type="text/css"
      id="dark-mode"
      rel="stylesheet"
      href=""
    />
    <style xmlns="" type="text/css" id="dark-mode-custom-style" />
    <g
      id="Group_63"
      data-name="Group 63"
      transform="translate(-446.571 -211.615)"
    >
      <path
        id="Path_54"
        data-name="Path 54"
        d="M504.547,265.443h-9.019a30.964,30.964,0,0,0-29.042-52.733,1.5,1.5,0,1,0,.792,2.894,27.955,27.955,0,0,1,25.512,48.253l0-10.169h-.011a1.493,1.493,0,0,0-2.985,0h0v13.255a1.5,1.5,0,0,0,1.5,1.5h13.256a1.5,1.5,0,1,0,0-3Z"
        fill="#0c2c67"
      />
      <path
        id="Path_55"
        data-name="Path 55"
        d="M485.389,267.995a27.956,27.956,0,0,1-25.561-48.213l0,10.2h.015a1.491,1.491,0,0,0,2.978,0h.007V216.791a1.484,1.484,0,0,0-1.189-1.532l-.018-.005a1.533,1.533,0,0,0-.223-.022c-.024,0-.046-.007-.07-.007H448.071a1.5,1.5,0,0,0,0,3h8.995a30.963,30.963,0,0,0,29.115,52.664,1.5,1.5,0,0,0-.792-2.894Z"
        fill="#0c2c67"
      />
    </g>
  </svg>
  `;
  const button = document.createElement("button");
  button.type = "button";
  button.innerHTML = ICON;
  button.title = "Recharger l'application";

  button.addEventListener("click", () => {
    sessionStorage.removeItem("dartAppInstanceId");
    location.reload();
  });
  Object.assign(button.style, {
    display: "grid",
    placeContent: "center",
    placeItems: "center",
    gridTemplate: "1fr / 1fr",
    aspectRatio: "1/1",
    zIndex: 9999,
    position: "fixed",
    bottom: "1rem",
    right: "1rem",
    height: "2rem",
    borderRadius: "calc(1px * infinity)",
    padding: 0,
  });
  Object.assign(
    //  svg icon
    button.children[0].style,
    {
      height: "65%",
      aspectRatio: "1/1",
    },
  );
  window.addEventListener("load", () => document.body.append(button));
}
